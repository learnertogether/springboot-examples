package com.zxl.examples.zookeeper;
...
public class SyncPrimitive implements Watcher {
    ...
    synchronized public void process(WatchedEvent event) {
        synchronized (mutex) {
            System.out.println("Process: " + event.getType()+":"+event.getPath()+":"+event.getState().getIntValue());
            mutex.notify();
        }
    }
            ...
            while (true) {
                synchronized (mutex) {
                    ...
                    if (list.size() == 0) {
                        ...
                        String minStr = new String(list.get(0).substring(7));
                        for(String s : list){
